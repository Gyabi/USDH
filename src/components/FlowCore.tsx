...
const FlowCore = () => {
  ...
  const [filePath, setFilePath] = useState(router.query.filePath);
  useEffect(() => {
    onRestore();
  }, [router.pathname])
  ...
  const onDrop = useCallback(
    ...
    const onSave = useCallback(() => {
      if (reactFlowInstance) {
        ...
        // Rust側で永続化
        invoke('write_file', {path:filePath, contents:JSON.stringify(flow)});
      }
    }, [reactFlowInstance]);
    ...
    const onRestore = useCallback(() => {
      const restoreFlow = async () => {
        // Rust側から読み出し
        invoke('read_file', {path:filePath}).then((message:string) => {
          // 新規作成されたファイルなら無視
          if(message != "{}")
          {
    ...
    const MoveHomePage = (path : string) => {
      Router.push({
        pathname:path,
      })
    }
    ...
    return (
      /* h-screenによって全画面に表示している */
        <div className="flex grow h-screen" ref={reactFlowWrapper}>
              ...
              <aside className="border-r-2 px-4 py-3 text-xs flex-col flex">
                  <button onClick={onSave} className="my-0.5 bg-gray-600 hover:bg-gray-500 text-white rounded px-4 py-2 items-center justify-center flex">SAVE</button>
                  <button onClick={_ => MoveHomePage("/home")} className="my-0.5 bg-gray-600 hover:bg-gray-500 text-white rounded px-4 py-2 items-center justify-center flex">HOME</button>
              </aside>
          </div>
    );
